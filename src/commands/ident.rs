...
pub async fn handle(server: &mut ChatServer, client: SocketAddr, json: &Value) -> Result<(), CommandError> {
    ...
    let (nickname, channels_being_viewed) = {
        let user = server.users.get(&username).ok_or(CommandError::InvalidUsername)?;
        (user.nickname.to_string(), user.get_viewed_channels())
    };

    let info = server.get_unauthed_connection(client).ok_or(CommandError::InvalidArguments)?;

    server.remove_unauth_connection(client);
    server.add_connection(client, username.to_string(), info.tx.clone()).await;

    let channels: Vec<serde_json::Value> = server.channels.values()
