...
impl ChatDatabase for SqliteChatDatabase {
    ...
    async fn register_user(&self, username: &str, password: &str) -> Result<i64, DatabaseError> {
        sodiumoxide::init()
            .ok()
            .ok_or(DatabaseError::PasswordHashError)?;
        let mut conn = self.pool.acquire().await?;
        let pwh = pwhash_interactive(password.as_bytes())
            .ok()
            .ok_or(DatabaseError::PasswordHashError)?;
    ...
    async fn verify_login(&self, username: &str, password: &str) -> Result<bool, DatabaseError> {
        ...
        if let Some(record) = res {
            let hashed_password = HashedPassword::from_slice(record.password.as_ref())
                .ok_or(DatabaseError::PasswordHashError)?;
    ...
    async fn get_users(&self) -> Result<MultiMap<String, i64, User>, DatabaseError> {
        ...
        let mut conn = self.pool.acquire().await?;
        let user_records = sqlx::query!("SELECT id, username, nickname FROM users;")
            .fetch_all(&mut conn)
            .await?;
    ...
    async fn get_channels(
        ...
        for record in channel_records {
            let chan = channels
                ...
                .or_insert_with(|| Channel::new(record.id, &record.name));
    ...
    async fn make_channel(&self, name: &str) -> Result<i64, DatabaseError> {
        let mut conn = self.pool.acquire().await?;
        let res = sqlx::query!("INSERT INTO channels VALUES (null, ?);", name)
            .execute(&mut conn)
            .await?;
        Ok(res.last_insert_rowid())
    }
    ...
    async fn get_last_message_read(
        ...
        let mut conn = self.pool.acquire().await?;
        let res = sqlx::query!(
            "SELECT message_id FROM user_last_read_messages WHERE user_id = ? AND channel_id = ?;",
            user_id,
            channel_id
        )
        .fetch_optional(&mut conn)
    ...
    async fn clear_last_message_read(
        ...
        let mut conn = self.pool.acquire().await?;
        sqlx::query!(
            "DELETE FROM user_last_read_messages WHERE user_id = ? AND channel_id = ?;",
            user_id,
            channel_id
        )
        .execute(&mut conn)
