            ...
            match v {
                Ok(packet) => match packet {
                    tungstenite::Message::Text(msg) => {
                        Poll::Ready(Some(ClientStreamMessage::WebSocketText(msg)))
                    }
                    tungstenite::Message::Ping(data) => {
                        Poll::Ready(Some(ClientStreamMessage::WebSocketPing(data)))
                    }
                    tungstenite::Message::Close(_) => {
                        Poll::Ready(Some(ClientStreamMessage::WebSocketClose))
                    }
                    _ => Poll::Ready(None),
                },
...
pub async fn process_client(
    ...
    let mut client_stream = ClientStream {
        manager_rx: mrx_stream,
        ws_rx,
        shutdown_rx: shutdown_stream,
    };

    // This will loop as long as the websocket is still connected.
    while let Some(result) = client_stream.next().await {
        match result {
            ClientStreamMessage::ManagerData(response) => match response {
                ServerCommandResponse::Text(msg) => {
                    ws_tx.send(tungstenite::Message::Text(msg)).await?
                }
                ServerCommandResponse::Disconnect(e) => {
                    ws_tx
                        .send(tungstenite::Message::Close(Some(error_to_close_frame(e))))
                        .await?;
                    break;
                }
            },
            ClientStreamMessage::WebSocketPing(data) => {
                ws_tx.send(tungstenite::Message::Pong(data)).await?
            }
            ClientStreamMessage::WebSocketText(data) => {
                tx.send(Message::NewData((addr, data))).await?
            }
            ClientStreamMessage::WebSocketClose => {
                tx.send(Message::Disconnected(addr)).await?;
                ws_tx
                    .send(tungstenite::Message::Close(Some(get_normal_close_frame())))
                    .await?;
            }
